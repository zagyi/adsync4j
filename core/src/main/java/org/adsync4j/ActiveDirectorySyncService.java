 ...
 * <p/>
 * Implementations have to consult a {@link DomainControllerAffiliation} record in order to get the details based on which the
 * synchronization operations are to be carried out.
 * <p/>
 * For more on the {@code LDAP_ATTRIBUTE} type parameter please refer to the documentation of {@link LdapClient} and {@link
 * LdapAttributeResolver}.
 *
 * @param <LDAP_ATTRIBUTE> An LDAP SDK specific attribute type. Determined by the {@link LdapClient} implementation in use.
 */
public interface ActiveDirectorySyncService<LDAP_ATTRIBUTE> {
     ...
     * Directory.<br>
     * Entries are reported to the caller by iteratively invoking {@link EntryProcessor#processNew processNew()} of the provided
     * {@link EntryProcessor}.
     *
